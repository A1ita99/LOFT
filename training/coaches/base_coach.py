...
# from models.psp.psp import mypSp
...
class BaseCoach:
    def __init__(self, data_loader, use_wandb):
        ...
        if hyperparameters.first_inv_type == 'w+':
            print("running in e4e")
            self.initilize_e4e()
        if hyperparameters.first_inv_type == 'psp':
            print("running in psp")
            self.initilize_psp()
    ...
    def load_inversions(self, w_path_dir, image_name):
        ...
        if hyperparameters.first_inv_type == 'w+':
            print("load:",f'{w_path_dir}/{paths_config.e4e_results_keyword}/{image_name}/0.pt')
            w_potential_path = f'{w_path_dir}/{paths_config.e4e_results_keyword}/{image_name}/0.pt'
        else:
            print("load:", f'{w_path_dir}/{paths_config.pti_results_keyword}/{image_name}/0.pt')
        ...
        print("get w and its shape is:",w.shape)
        return w
    ...
    def calc_inversions(self, image, image_name):
        ...
        elif hyperparameters.first_inv_type == 'psp':
            w = self.get_psp_inversion(image)
    ...
    def initilize_psp(self):
        ckpt = torch.load(paths_config.psp, map_location='cpu')
        opts = ckpt['opts']
        opts['batch_size'] = hyperparameters.train_batch_size
        opts['checkpoint_path'] = paths_config.psp
        ...
        opts = Namespace(**opts)
        self.psp_inversion_net = pSp(opts)
        self.psp_inversion_net.eval()
        self.psp_inversion_net = self.psp_inversion_net.to(global_config.device)
        toogle_grad(self.psp_inversion_net, False)

    def get_e4e_inversion(self, image):
        print("getting_e4e_inversion")
        image = (image + 1) / 2
    ...
    def get_psp_inversion(self, image):
        print("getting_psp_inversion")
        image = (image + 1) / 2
        new_image = self.e4e_image_transform(image[0]).to(global_config.device)
        _, w = self.psp_inversion_net(new_image.unsqueeze(0), randomize_noise=False, return_latents=True, resize=False,
                                      input_code=False)
        if self.use_wandb:
            log_image_from_w(w, self.G, 'First psp inversion')
        return w